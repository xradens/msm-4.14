...
ZIPNAME="WM-surya-$(date '+%Y%m%d-%H%M').zip"
...
if [[ $1 = "-u" || $1 = "--update" ]]; then
    sync_repo $AK3_DIR "https://github.com/xradens/AnyKernel3.git" "oss" true
    sync_repo $TC_DIR "https://bitbucket.org/rdxzv/clang-standalone.git" "20" true
	exit
else
    sync_repo $AK3_DIR "https://github.com/xradens/AnyKernel3.git" "oss" false
...
for arg in "$@"; do
	case $arg in
		...
		-s|--su)
			ENABLE_KSU=true
			ZIPNAME="${ZIPNAME/WM-surya/WM-KSUN}"
			;;
		*)
